...
WEBAPP_HOST = "0.0.0.0"
WEBAPP_PORT = 8080
# ⚠️ ВАЖНО: укажите реальный публичный URL вашего сервера (https://ваш-домен или ngrok)
# Если запускаете локально для тестирования – используйте ngrok и пропишите его сюда
WEBAPP_URL = "https://telegram.premium.bothost.tech"   # Замените на свой адрес!
...
<style>
/* (весь CSS из исходного кода, без изменений) */
:root {
...
.promo-banner p { font-size: 13px; opacity: 0.9; }
.categories-tabs {
    display: flex;
    gap: 8px;
    padding: 12px 16px;
...
.product-price .currency { font-size: 14px; font-weight: 500; color: var(--tg-theme-hint-color); }
.buy-btn {
    padding: 8px 20px;
    border-radius: 20px;
    border: none;
...
.promo-input:focus { border-color: var(--tg-theme-button-color); }
.promo-apply-btn {
    padding: 12px 20px;
    border-radius: 12px;
    border: none;
...
.payment-screen.active { display: block; }
.payment-loader {
    width: 48px;
    height: 48px;
...
.payment-link-btn:active { transform: scale(0.98); }
.check-btn {
    display: block;
    width: 100%;
    padding: 14px;
...
<div class="modal-overlay" id="productModal" onclick="closeModalOutside(event)">
    <div class="modal-content" id="modalContent">
        ...
        <div id="modalProductView">
            ...
            <div class="promo-section">
                <div class="promo-input-wrap">
                    <input type="text" class="promo-input" id="promoInput" placeholder="Промокод" autocapitalize="characters">
        ...
        <div class="payment-screen" id="paymentScreen">
            ...
            <button class="check-btn" onclick="cancelPayment()" style="border-color: var(--tg-theme-destructive-text-color); color: var(--tg-theme-destructive-text-color); margin-top: 8px;">
                ❌ Отменить
            </button>
        </div>
...
<div class="modal-overlay" id="promoModal" onclick="closePromoModal(event)">
    <div class="modal-content" style="max-height: 40vh;">
        ...
        <div style="padding: 20px;">
            ...
            <div class="promo-input-wrap">
                <input type="text" class="promo-input" id="globalPromoInput" placeholder="Введите промокод" autocapitalize="characters">
...
async function loadOrders() {
    ...
    try {
        ...
        for (const order of data.orders) {
            const statusClass = order.status === 'success' ? 'success' : (order.status === 'pending' ? 'pending' : 'expired');
            const statusText = order.status === 'success' ? 'Оплачен' : (order.status === 'pending' ? 'Ожидает' : 'Истёк');
            html += `
...
async function applyPromo() {
    ...
    try {
        const resp = await fetch(`${API_BASE}/api/promo/check`, {
            ...
            body: JSON.stringify({ code: code, amount: selectedProduct.price, initData: tg.initData || '' })
        });
        const data = await resp.json();
        if (data.valid) {
            ...
            document.getElementById('modalPrice').innerHTML = `<s style="color:var(--tg-theme-hint-color);font-size:18px;">${selectedProduct.price} ₽</s> ${newPrice} ₽`;
...
async function createPayment() {
    ...
    if (appliedDiscount > 0) price = Math.max(1, Math.round(price * (100 - appliedDiscount) / 100));
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('paymentInfo').innerHTML = `📦 ${selectedProduct.name}<br>💰 ${data.amount} ₽<br><br>Оплатите по ссылке и нажмите "Проверить"`;
...
async function checkPayment() {
    ...
    try {
        const resp = await fetch(`${API_BASE}/api/payment/check`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ payment_id: currentPaymentId, initData: tg.initData || '' })
        });
        const data = await resp.json();
        if (data.paid) {
...
def extract_user_id(init_data_str):
    ...
    # Fallback для разработки (если нет валидации)
    try:
        parsed = dict(x.split('=', 1) for x in init_data_str.split('&'))
        user_data = json.loads(unquote(parsed.get('user', '{}')))
        return user_data.get('id')
...
async def api_promo_check(request):
    try:
        ...
        return web.json_response({"valid": False, "message": "Некорректный запрос"})
...
async def api_create_payment(request):
    try:
        ...
        return web.json_response({"success": False, "message": "Некорректный запрос"})
...
async def api_check_payment(request):
    try:
        ...
        return web.json_response({"paid": False, "message": "Некорректный запрос"})
...
async def on_startup(app_web):
    ...
    if not BOT_TOKEN:
        ...
        return
    if not WEBAPP_URL or WEBAPP_URL == "https://telegram.premium.bothost.tech":
        logger.warning("WEBAPP_URL не настроен! Используется заглушка. Укажите реальный URL вашего сервера.")
